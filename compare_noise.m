...
% unifrnd(-0.5,0.5,2*L,1)+ 1j*unifrnd(-0.5,0.5,2*L,1);
%       u1 = unifrnd(-0.5,0.5,2*L,1)+ 1j*unifrnd(-0.5,0.5,2*L,1);
        Input = sqrt(0.5)*(randn(1,N) + 1j*randn(1,N)); 
%        Input = sqrt(0.4)*randn(1,N) + sqrt(0.6)*1j*randn(1,N);
...
%°≠°≠°≠°≠°≠°≠°≠°≠°≠°≠À„∑®°≠°≠°≠°≠°≠°≠°≠°≠°≠°≠
    ...
    [Da]= ACLMS(Input,Output,D,unknown_w,Lw,0.04); 
    MSDg = MSDg + Da.MSD;
%     [Da]= ACLMK(Input,Output,D,unknown_w,Lw,0.0003);
%     MSDy = MSDy + Da.MSD;
    [Da]= E1(Input,Output,D,unknown_w,Lw,0.05,0.5);
    MSDy = MSDy + Da.MSD;
    ...
    MSDm = MSDm + Da.MSD;
    [Da]= NCCHS2(Input,Output,noise,D,unknown_w,Lw,0.8,0.5,0.03,-20);
    MSDb = MSDb + Da.MSD;
end
...
hold on;
plot(n,10*log10(MSDy(n)),'color','[0,0.4,0]','linewidth',1);
hold on;
...
h=legend('WL-CLMS \mu=0.04','LHCAF \mu=0.05 \lambda=0.5','WL-ILSE \mu=0.02 \beta=2','MICCC \mu=1.5 \sigma=2 \rho=0.2','WL-CHSAF \mu=0.04 \lambda=0.8', 'WL-NCAFHS \mu=0.03 \tau=0.8 \beta=0.5 \gamma=-20');
